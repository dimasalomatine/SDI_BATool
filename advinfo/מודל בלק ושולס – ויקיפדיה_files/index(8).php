...
function InitImgToggle()
{
  ...
  try {
    ...
    divWrapper = mw.util.$content.find('div.img_toggle');
    if ( !divWrapper.length ) return;

    for ( var i = 0 ; i < divWrapper.length ; i++ )
    {
        var boxes = getElementsByClassName (divWrapper[i], "div", "imgtogglebox" );
        ...
        toggleRight.onclick = function()
            {
              try {
                ...
                var boxes = getElementsByClassName ( this.parentNode.parentNode, "div", "imgtogglebox" );
